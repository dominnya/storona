...
test("Handles wrong method in path", () => {
  expect([
    "Failed to register tests\\routes\\nested\\wrong-method.pet.ts: Method must be one of: all, get, post, put, delete, patch, options, head",
    "Failed to register tests/routes/nested/wrong-method.pet.ts: Method must be one of: all, get, post, put, delete, patch, options, head",
  ]).toContain(debug.logs[13]);
});

test("Handles wrong method in export", () => {
  expect(debug.logs[12]).toBe(
    "Failed to register /nested/wrong-method-overr.ide: Exported method must be one of: all, get, post, put, delete, patch, options, head",
  );
...
test("Throws an error when override is of different type", () => {
  ...
  expect(debug.logs[5]).toBe(
    "Failed to register /nested/nested/!wrong-method: Exported method must be one of: all, get, post, put, delete, patch, options, head",
  );
});

test("Throws a warning when overriden route does not start with /", () => {
  expect(debug.logs[4]).toBe(
    "Registered GET /nested/nested/warning-override",
  );
